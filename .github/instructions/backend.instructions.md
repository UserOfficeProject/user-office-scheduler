...
- Implement both PostgreSQL and mock data sources for each DataSource interface

### Architecture
- Implement GraphQL resolvers using TypeGraphQL decorators
- Keep business logic in the mutations and queries layers, separate from resolvers
...
- Unauthorized query requests should return null, not throw error
- Unauthorized mutation requests should log the attempt and return an error message without throwing an exception
...
### Testing
- Mock external dependencies using mocks
